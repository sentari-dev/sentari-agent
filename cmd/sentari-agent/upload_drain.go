...
func runUpload(ctx context.Context, client *comms.Client, auditLog *audit.AuditLog, scanCache *cache.Cache, agentCfg config.AgentConfig, cp cycleParams, igDisableDebounce *comms.InstallGateDisableDebouncer) error {
	...
	// + auto-detected runtime.  Both shipped on every /scan for the
	// server-side tag-filtering and runtime propose-then-approve
	// features.  Set here rather than inside scanner.Run so the scanner
	// package stays free of agent-config + runtime-detect awareness.
