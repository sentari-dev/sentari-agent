...
// linkage gives us that for free, and the server's canonical-JSON
// signer produces byte-identical output
...
// the server's canonical-JSON signer; an envelope larger than this is
...
func VerifyInstallGateEnvelope(data []byte) (*InstallGateMap, error) {
	...
	// the server's canonical-JSON serializer.  The helper decodes
