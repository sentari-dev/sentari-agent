...
// TestDiscoverInRoot_classifiesYarnBerry proves a yarn.lock that opens
// with a `__metadata:` block (yarn v2+/berry) is classified as
// "yarn_berry", not "yarn_v1" — the v1 parser would otherwise emit
// garbage from the berry format.
func TestDiscoverInRoot_classifiesYarnBerry(t *testing.T) {
	root := t.TempDir()
	mustWrite(t, filepath.Join(root, "yarn.lock"), `# This file is generated by running "yarn install" inside your project.
...
`)
	...
	if len(results) != 1 {
		t.Fatalf("expected 1 lockfile, got %d: %+v", len(results), results)
	}
	if results[0].Format != "yarn_berry" {
		t.Errorf("expected format yarn_berry, got %q", results[0].Format)
	}
	if results[0].Ecosystem != "npm" {
		t.Errorf("expected ecosystem npm, got %q", results[0].Ecosystem)
	}
}

// TestDiscoverInRoot_classifiesYarnV1 keeps the classic (v1) yarn.lock
// classified as yarn_v1.
func TestDiscoverInRoot_classifiesYarnV1(t *testing.T) {
	root := t.TempDir()
